  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <div>
          <p style={{ color: '#64748b', fontSize: 11, margin: '0 0 4px' }}>📍 {data?.region}</p>
          <span style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <p style={{ color: '#64748b', fontSize: 10, margin: 0 }}>PM2.5 ㎍/㎥</p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        {[
          { label: 'PM10', val: fmt(data?.pm10, ' ㎍/㎥') },
          { label: 'CO₂', val: fmt(data?.co2, ' ppm') },
          { label: '온도', val: fmt(data?.temperature, '°C') },
          { label: '습도', val: fmt(data?.humidity, '%') },
        ].map(({ label, val }) => (
          <div key={label} style={{ background: '#0f172a', borderRadius: 8, padding: '8px 10px' }}>
            <p style={{ color: '#475569', fontSize: 10, margin: '0 0 2px' }}>{label}</p>
            <p style={{ color: '#cbd5e1', fontSize: 13, fontWeight: 600, margin: 0, fontFamily: 'monospace' }}>{val}</p>
          </div>
        ))}
      </div>

      {data?.measured_at && (
        <p style={{ color: '#334155', fontSize: 10, margin: '10px 0 0', textAlign: 'right' }}>
